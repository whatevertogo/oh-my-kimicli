		...
		for (const tool of turn.tools || []) {
			...
			lines.push(
				record(timestamp++, "ToolResult", {
					...
					return_value: {
						is_error: Boolean(tool.error),
						content: tool.error || "ok",
					},
				}),
			);
		}
...
test("insights no-llm report is generated from Kimi sessions", () =>
	withTempHomes(async (dir, env) => {
		...
		assert.match(
			readFileSync(report.reportHtmlPath, "utf8"),
			/oh-my-kimicli insights/,
		);
		assert.equal(report.mode, "metrics-only");
		assert.equal(report.schema_version, 2);
		assert.doesNotMatch(
			readFileSync(report.reportHtmlPath, "utf8"),
			/brand-mark|prompt-card|在地平线上|Skill 机会|有趣的结尾/,
		);
	}));
...
test("insights message writes manifest task and session packets", () =>
	withTempHomes(async (dir, env) => {
		...
		const input = await collectInsightsMessage({
			env,
			limit: 20,
			facetLimit: 10,
		});
		...
		const packets = readdirSync(paths.sessionsDir).filter((name) =>
			name.endsWith(".md"),
		);
		const packetText = readFileSync(
			join(paths.sessionsDir, packets[0]),
			"utf8",
		);
		...
		assert.equal(
			payload.aggregate_metrics.workflowSignals.prompt_intents.implementation,
			1,
		);
...
test("insights message prefers the user's Chinese language for narrative output", () =>
	withTempHomes(async (dir, env) => {
		...
		assert.equal(
			payload.ranked_sessions[0].workflow_tags.includes("review"),
			true,
		);
	}));
...
test("insights skill frontmatter is discoverable and uses v2 workflow", () => {
	const text = readFileSync(
		join(import.meta.dir, "..", "skills", "insights", "SKILL.md"),
		"utf8",
	);
...
test("insights cli supports message paths and metrics commands only", () =>
	withTempHomes(async (dir, env) => {
		...
		await runInsightsCli(["message", "--limit", "20"], {
			env,
		...
		await assert.rejects(
			() => runInsightsCli(["render"], { env, stdout: () => {} }),
			/Unknown insights command: render/,
		);
		await assert.rejects(
			() => runInsightsCli(["collect"], { env, stdout: () => {} }),
			/Unknown insights command: collect/,
		);
	}));
