...
export function buildSessionMeta(session, turns) {
  ...
  const allTimestamps = allEvents.map((event) => event.timestamp).filter(Boolean);
  const start = allTimestamps.length ? Math.min(...allTimestamps) : session.wireMtimeMs / 1000;
  ...
  let gitCommits = 0;
  let gitPushes = 0;
  let usesSubagent = turns.some((turn) => turn.events.some((event) => event.source === "subagent"));
  let usesMcp = false;
  let usesWeb = false;
  ...
  for (const turn of visibleTurns) {
    ...
    for (const event of turn.events) {
      if (event.type === "TurnBegin" && event.source === "root") {
        ...
        const fn = isObject(event.payload.function) ? event.payload.function : {};
        ...
        const rv = isObject(event.payload.return_value) ? event.payload.return_value : {};
        if (rv.is_error) {
          toolErrorCount += 1;
        ...
        lastAssistantTimestamp = event.timestamp;
      } else if (event.type === "ApprovalResponse" && event.payload.response === "reject") {
        toolErrorCount += 1;
        toolErrorCategories["User Rejected"] = (toolErrorCategories["User Rejected"] || 0) + 1;
      } else if (event.type === "StatusUpdate") {
        const usage = isObject(event.payload.token_usage) ? event.payload.token_usage : {};
...
function estimateLineDiff(args) {
  ...
  const newText = String(args.new_str || args.new_string || args.newText || args.content || "");
  return {
