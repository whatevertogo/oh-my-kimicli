export function defaultSections(aggregated = {}) {
	...
	const featureSuggestions = defaultFeatureSuggestions(
		data,
		useChinese ? "zh" : "en",
	);
	const usagePatterns = defaultUsagePatterns(data, useChinese ? "zh" : "en");
	const skillOpportunities = defaultSkillOpportunities(
		data,
		useChinese ? "zh" : "en",
	);
	if (useChinese) {
		return {
			...
			what_works: {
				intro:
					"从指标看，你已经有一些稳定有效的协作方式；叙事版报告会进一步解释这些模式为什么有效。",
			...
			friction_analysis: {
				...
				categories: mapEntries(
					data.toolErrorCategories,
					3,
					(category, count) => ({
						category,
	...
	return {
		...
		what_works: {
			intro:
				"The metrics already show a few stable collaboration patterns; the narrative skill can explain why they work.",
			impressive_workflows: workflowItems,
		},
		friction_analysis: {
			intro:
		...
		on_the_horizon: {
			intro:
				"Future reports can connect usage patterns to higher-level automation instead of only showing counts.",
		...
		items.push(
			language === "zh"
				? {
						title: "数据仍在积累",
						description:
							"当前 session 还不够多，但已经能看到工具使用、项目分布和响应节奏。多跑几次后，叙事版报告会更有判断力。",
					}
				: {
						title: "Evidence is still forming",
						description:
							"There are not enough sessions for a strong pattern yet, but tool usage, project distribution, and response cadence are already measurable.",
					},
		);
	}
...
function instructionAdditions(data, language) {
	...
	return [
		language === "zh"
			? {
					addition:
						"把重复出现的审查、验证、计划偏好沉淀到 AGENTS.md 或 skill 中。",
					why: "当前没有检测到完全重复的首轮指令，但任务意图已经显示出稳定偏好。",
					prompt_scaffold:
						"请根据最近的 insights 报告，帮我把重复偏好整理成 AGENTS.md 或 skill 规则。",
				}
			: {
					addition:
						"Move repeated review, verification, and planning preferences into AGENTS.md or skills.",
					why: "No exact repeated first prompt was detected yet, but the intent distribution already shows stable preferences.",
					prompt_scaffold:
						"Use the latest insights report to turn repeated preferences into AGENTS.md or skill rules.",
				},
	];
}
	...
	const suggestions = [
		language === "zh"
			? {
					...
					why_for_you:
						"它避免嵌套 kimi --print，也能基于真实 session 反推出该沉淀哪些 skill/hook/AGENTS 指令。",
					example_code: "/skill:insights",
				}
			: {
					feature: "/skill:insights",
					one_liner:
						"Let the current Kimi agent write the narrative report directly.",
					why_for_you:
						"It avoids nested kimi --print and can turn real sessions into skill, hook, or AGENTS.md opportunities.",
					example_code: "/skill:insights",
				},
	];
		...
		suggestions.push(
			language === "zh"
				? {
						...
						why_for_you:
							"review 是高频意图时，固定格式和证据位置能减少每次重新解释标准。",
				...
				: {
						...
						why_for_you:
							"When review is frequent, fixed evidence and output locations reduce repeated setup.",
						example_code: "/skill:omk-review",
					},
		);
	}
	if (
		(intents.autonomous_execution || 0) > 0 ||
		(mentions.ralph || 0) > 0 ||
		(mentions.ultrawork || 0) > 0
	) {
		suggestions.push(
			language === "zh"
				? {
						...
						why_for_you:
							"你的长任务里已经出现 Ralph/ultrawork 信号，适合把完成标准和验证证据固定下来。",
				...
				: {
						...
						why_for_you:
							"Your longer sessions already mention Ralph or ultrawork, so completion criteria and evidence should be explicit.",
	...
	const patterns = [
		language === "zh"
			? {
					...
					detail:
						"当任务横跨多个模块时，先确认计划能减少后续反复修正；小任务仍可直接执行。",
					copyable_prompt:
						"先进入 plan mode，探索项目后给我一份可执行计划，确认后再改代码。",
				}
			: {
					title: "Plan large tasks first",
					suggestion: "Use plan mode for multi-file or architectural work.",
					detail:
						"When a task crosses module boundaries, a short approved plan reduces later correction; small tasks can still go straight to execution.",
					copyable_prompt:
						"Enter plan mode first, inspect the project, and give me an executable plan before editing.",
				},
	];
	if (errors > 0) {
		patterns.push(
			language === "zh"
				? {
						title: "把失败转成下一步证据",
						suggestion:
							"工具失败时要求 agent 记录失败原因和替代验证，而不是直接跳过。",
						detail: `${errors} 次工具错误说明 report 应该把失败也当作工作流信号，用来优化 hooks 或 skill。`,
						copyable_prompt:
							"如果工具失败，请记录失败命令、原因、替代检查，然后继续完成任务。",
					}
				: {
						title: "Turn failures into evidence",
						suggestion:
							"Ask the agent to record failed commands and alternate checks instead of skipping them.",
						detail: `${errors} tool errors mean failures should feed skill or hook improvements, not disappear from the workflow.`,
						copyable_prompt:
							"If a tool fails, record the command, reason, alternate check, and continue the task.",
					},
		);
	}
		...
		opportunities.push(
			language === "zh"
				...
				: {
						name: "review-output-discipline",
						trigger:
							"when the user asks for review, risk checks, or pre-commit validation",
						why: "Review is a reusable workflow that benefits from fixed output paths, evidence format, and verification rules.",
						evidence: [`review intent: ${intents.review}`],
						proposed_scope:
							"Update omk-review or create a narrower project-review skill.",
		...
		opportunities.push(
			language === "zh"
				? {
						...
						evidence: [
							`autonomous_execution intent: ${intents.autonomous_execution}`,
						],
				...
				: {
						name: "autonomous-completion-discipline",
						trigger:
							"when the user asks for Ralph, ultrawork, or keep-going execution",
						why: "Long tasks need explicit done/blocked state, verification evidence, and continuation rules.",
						evidence: [
							`autonomous_execution intent: ${intents.autonomous_execution}`,
						],
						proposed_scope:
							"Tighten ultrawork and omk-ralph coordination prompts.",
		...
		opportunities.push(
			language === "zh"
				...
				: {
						...
						proposed_scope:
							"Collect more sessions and decide in the next report.",
		...
		return [
			{
				...
				whats_possible:
					"当报告连续发现同类摩擦时，下一步不只是阅读建议，而是让 agent 直接起草 skill、hook 或 AGENTS.md 规则。",
				how_to_try:
					"运行 /skill:insights 后，只确认一个最强候选，让 agent 基于真实 session 数据改插件。",
				copyable_prompt:
					"基于这份 insights 报告，先只实现最有证据的一个 skill/hook 改进。",
			},
			{
				title: "计划模式与 Ralph 串联",
				whats_possible:
					"大任务先 plan，再由 Ralph/ultrawork 持续执行和验证，可以减少半途总结与遗漏。",
				how_to_try: "对多文件任务要求先 EnterPlanMode，确认后进入完成循环。",
				copyable_prompt:
					"这是一个大任务：先 plan，确认后用 Ralph/ultrawork 持续执行到 done 或 blocked。",
			},
			{
				title: "围绕高频工具优化提示",
				...
				copyable_prompt:
					"根据 insights 的高频工具和错误类型，帮我优化一个最小的提示规则。",
			},
		];
	}
	return [
		{
			...
			whats_possible:
				"When reports repeatedly find the same friction, the next step can be drafting a skill, hook, or AGENTS.md rule from real session evidence.",
			how_to_try:
				"Run /skill:insights, confirm one strongest candidate, and let the agent update the plugin.",
			copyable_prompt:
				"Based on this insights report, implement only the strongest evidence-backed skill or hook improvement.",
		},
		{
			title: "Plan mode plus Ralph",
			whats_possible:
				"Large tasks can start with plan approval, then continue through Ralph or ultrawork until verified done or blocked.",
			how_to_try:
				"Ask for EnterPlanMode on multi-file tasks, then move into a completion loop after approval.",
			copyable_prompt:
				"This is a large task: plan first, then use Ralph/ultrawork until done or blocked.",
		},
		{
			...
			how_to_try:
				"Use the most frequent tools and error categories as hook or skill optimization input.",
			copyable_prompt:
				"Use the insights top tools and error categories to propose one minimal prompt rule.",
		},
	];
}
...
function topIntent(data) {
	return (
		Object.keys(data.workflowSignals?.prompt_intents || {})[0] || "general"
	);
}
...
export function normalizeSections(value, aggregated = {}) {
	...
	const sections = {
		...
		at_a_glance: {
			whats_working: text(
				value.at_a_glance?.whats_working,
				fallback.at_a_glance.whats_working,
			),
			whats_hindering: text(
				value.at_a_glance?.whats_hindering,
				fallback.at_a_glance.whats_hindering,
			),
			quick_wins: text(
				value.at_a_glance?.quick_wins,
				fallback.at_a_glance.quick_wins,
			),
		...
		project_areas: {
			areas: array(
				value.project_areas?.areas,
				fallback.project_areas.areas,
			).map((area) => ({
		...
		interaction_style: {
			narrative: text(
				value.interaction_style?.narrative,
				fallback.interaction_style.narrative,
			),
			key_pattern: text(
				value.interaction_style?.key_pattern,
				fallback.interaction_style.key_pattern,
			),
		},
		what_works: {
		...
		friction_analysis: {
			intro: text(
				value.friction_analysis?.intro,
				fallback.friction_analysis.intro,
			),
			categories: array(
				value.friction_analysis?.categories,
				fallback.friction_analysis.categories,
			).map((item) => ({
		...
		suggestions: {
			...
			features_to_try: array(
				value.suggestions?.features_to_try,
				fallback.suggestions.features_to_try,
			).map((item) => ({
			...
			usage_patterns: array(
				value.suggestions?.usage_patterns,
				fallback.suggestions.usage_patterns,
			).map((item) => ({
		...
		skill_opportunities: array(
			value.skill_opportunities,
			fallback.skill_opportunities,
		).map((item) => ({
