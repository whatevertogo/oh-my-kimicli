...
export function scanSessions(env = process.env) {
	...
	if (existsSync(root)) {
		for (const workDirEntry of readdirSync(root, { withFileTypes: true })) {
			...
			for (const sessionEntry of readdirSync(workDirRoot, {
				withFileTypes: true,
			})) {
	...
	if (existsSync(importedRoot)) {
		for (const sessionEntry of readdirSync(importedRoot, {
			withFileTypes: true,
		})) {
			if (!sessionEntry.isDirectory()) {
				continue;
			...
			const session = scanSessionDir(
				join(importedRoot, sessionEntry.name),
				"__imported__",
				"",
				true,
			);
...
function readWorkDirMap(env) {
	...
	try {
		...
		for (const item of Array.isArray(metadata.work_dirs)
			? metadata.work_dirs
			: []) {
