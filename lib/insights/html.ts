...
function whatWorks(section = {}, labels) {
	const workflows = Array.isArray(section.impressive_workflows)
		? section.impressive_workflows
		: [];
...
function workflowSignals(signals = {}, labels) {
    ...
    ${rows
			.map(
				([label, value]) =>
					`<p><strong>${escapeHtml(label)}:</strong> ${escapeHtml(formatNumber(value || 0))}</p>`,
			)
			.join("")}
  </section>`;
}

function _paragraph(label, body, labels) {
...
function bulletList(items) {
	const list = Array.isArray(items) ? items.filter(Boolean) : [];
	return list.length
		? `<ul class="list">${list.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`
		: "";
}
...
function htmlLabels(data) {
	if (data.userLanguage?.code !== "zh") {
		return {
			...
			skillOpportunityNote:
				"Candidates are suggestions only. Apply them after confirming the scope.",
			trigger: "Trigger",
			why: "Why",
			scope: "Scope",
			risk: "Risk",
			...
			weakEvidence:
				"Evidence is weak here; run /skill:insights after more substantial sessions for a richer narrative.",
			notAvailable: "n/a",
		};
	}
	return {
		...
		weakEvidence:
			"这里证据偏弱；多积累几个实质性 session 后再运行 /skill:insights 会更丰富。",
		notAvailable: "暂无",
	};
}
