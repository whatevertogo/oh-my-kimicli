...
function confirmPlanModeCandidate(sessionId, input) {
	...
	if (
		...
		/Plan mode (activated|on)/i.test(output)
	) {
...
async function reconcilePlanModeCandidate(sessionId, input) {
	...
	if (kimiState.plan_mode !== candidate.expected_plan_mode) {
		clearConditionalPrompt(
			sessionId,
			source,
			"plan mode state did not match expected value",
		);
		return true;
	}
...
async function waitForConditionalPostSuccess(sessionId) {
	...
	for (let i = 0; i < 6; i += 1) {
		...
		if (
			!state.conditional_prompt?.require_post_success ||
			state.conditional_prompt.post_success
		) {
			return state;
		}
	}
...
function renderPrompt(text) {
	return text.replace(/\{\{PLAN_TEMPLATE\}\}/g, () =>
		readPromptPartial("plan/plan-template.md"),
	);
}
...
function renderRalphPrompt(prompt, state) {
	return prompt
		...
		.replace(
			/\{\{SKILL_SELECTION_STATUS\}\}/g,
			String(state.skill_selection_status || "n/a"),
		)
		.replace(/\{\{PLAN_STATUS\}\}/g, String(state.plan_status || "n/a"))
		.replace(
			/\{\{SELECTED_SKILLS\}\}/g,
			renderSelectedSkills(state.selected_skills),
		)
		.replace(
		...
		.replace(
			/\{\{MAX_ITERATIONS\}\}/g,
			renderMaxIterations(state.max_iterations),
		)
...
function gitAbsoluteDir(cwd) {
	try {
		return execFileSync(
			"git",
			["-C", cwd || process.cwd(), "rev-parse", "--absolute-git-dir"],
			{
				encoding: "utf8",
				timeout: 2000,
...
function readStdinJson() {
	if (process.stdin.isTTY) {
		console.error(
			"oh-my-kimicli hook expects JSON on stdin; allowing because stdin is a TTY.",
		);
		return null;
	}
