...
- npm 页面：<https://www.npmjs.com/package/@whatevertogo/oh-my-kimicli>
...
也可以用 npm 安装，但本包运行 `omk` 时仍需要本机已安装 Bun：
...
为什么不只运行 `bun install -g @whatevertogo/oh-my-kimicli`？

Bun 对 global package 有时会保留旧解析结果。你可能以为自己更新了，但 `bun pm ls -g` 仍显示旧版本或旧来源。`omk update` 通过先 remove 再 install 规避这个问题，并自动刷新已安装的 skill/hook。
